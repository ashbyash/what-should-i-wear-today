...
export default function LocationHeader({
  ...
  return (
    ...
    >
      ...
      {/* 내 위치로 돌아가기 + 업데이트 시간 (2열 레이아웃) */}
      {!isLoading && (
        <div
          className={`flex items-center mt-2 ${
            isViewingOtherLocation ? 'justify-center gap-3' : 'justify-center'
          }`}
        >
          {/* 내 위치로 돌아가기 (다른 위치 볼 때만) */}
          {isViewingOtherLocation && onReturnToCurrentLocation && (
            <button
              onClick={onReturnToCurrentLocation}
              className="flex items-center gap-1 px-3 py-1 rounded-full
                         text-xs text-glass-secondary
          ...
          {/* 구분자 (다른 위치 볼 때만) */}
          {isViewingOtherLocation && lastUpdated && (
            <span className="text-glass-muted/50">·</span>
          )}
